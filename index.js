...
*/
function getArtistByIndex(artistArray, artistArrayIndex) {
   return `The artist at index ${artistArrayIndex} is ${artistArray[artistArrayIndex].name}.`
  }
...
function lotsOfArt(artistArray) {
  let moreThan100Array = [];
  for (i = 0; i < artistArray.length; i++) {
    let numberOfPaintings = artistArray[i].paintings;
    if (numberOfPaintings > 100) {
      moreThan100Array.push(artistArray[i].name);
    }
  }
  return moreThan100Array;
}
...
function randomize(artistArray) {
  let startingIndex = artistArray.length, startingValue, randomIndex;
  while (0 !== startingIndex) {
    randomIndex = Math.floor(Math.random() * startingIndex);
    startingIndex -= 1;
    startingValue = artistArray[startingIndex];
    artistArray[startingIndex] = artistArray[randomIndex];
    artistArray[randomIndex] = startingValue;
  }
  return artistArray;
}
